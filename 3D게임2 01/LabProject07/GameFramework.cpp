...
void CGameFramework::BuildObjects()
{
	...
	pTerrainPlayer->SetOOBB(XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(2.0f, 2.0f, 2.0f), XMFLOAT4(0, 0, 0, 1));
...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		if (pKeysBuffer[0x4E] & 0xF0) //N키 - 달
		{
			m_pScene->m_pMoon->m_fCeta = 3.24;
			m_pScene->m_pMoon_1->m_fCeta = 3.24;
			//cout << "!" << endl;
		}
		if (pKeysBuffer[0x44] & 0xF0) //D키 - 낮
		{
			m_pScene->m_pMoon->m_fCeta = 0;
			m_pScene->m_pMoon_1->m_fCeta = 0;
		}
		if (pKeysBuffer[VK_SPACE])
		{
			if (m_bShootState == false)
			{
				if (m_pScene->getLights()->m_pLights[1].m_bEnable)
				{
					m_pScene->getLights()->m_pLights[1].m_bEnable = false;
					m_bShootState = true;
				}
		...
		{
			m_pScene->getLights()->m_pLights[1].m_bEnable = true;
			m_bShootState = false;
	...
	//cout << m_pPlayer->GetGravity().x << "\t" << m_pPlayer->GetGravity().y << m_pPlayer->GetGravity().z << endl;
}
