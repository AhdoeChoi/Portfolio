...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dDescriptorRanges[1].BaseShaderRegister = 1; //t1: gtxtTextures[6] (SRV니까 t0부터 6개를 쓰는 레지스터가있어 , 그놈의 정체가 gtxtTextures[6] 이거임)
	pd3dDescriptorRanges[1].RegisterSpace = 0;
	pd3dDescriptorRanges[1].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[2].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[2].NumDescriptors = 1;
	pd3dDescriptorRanges[2].BaseShaderRegister = 7; //t7: gtxtTerrainBaseTexture     (지형을 그릴때 베이스텍스쳐를 위한) t6에 연결
	pd3dDescriptorRanges[2].RegisterSpace = 0;
	pd3dDescriptorRanges[2].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[3].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[3].NumDescriptors = 1;
	pd3dDescriptorRanges[3].BaseShaderRegister = 8; //t8: gtxtTerrainDetailTexture           //t7에 연결
	pd3dDescriptorRanges[3].RegisterSpace = 0;
	pd3dDescriptorRanges[3].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[4].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[4].NumDescriptors = 1;
	pd3dDescriptorRanges[4].BaseShaderRegister = 9; //t9: gtxtSkyBoxTexture           //스카이박스를 위해 srv또 하나 연결될꺼야 
	...
	pd3dRootParameters[7].ParameterType = D3D12_ROOT_PARAMETER_TYPE_SRV; //쉐이더 리소스뷰로 게임오브젝트를 연결할꺼야 (t를씀)
	pd3dRootParameters[7].Descriptor.ShaderRegister = 0; //t0
	pd3dRootParameters[7].Descriptor.RegisterSpace = 0;
	pd3dRootParameters[7].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;

	D3D12_STATIC_SAMPLER_DESC pd3dSamplerDescs[2]; //스테틱 샘플러 2개만듬
	...
	pd3dSamplerDescs[0].ShaderRegister = 0; //s0에 연결하니까 0 gWrapSamplerState
	...
	pd3dSamplerDescs[1].ShaderRegister = 1; //s1에 연결할꺼니 s1 gClampSamplerState
