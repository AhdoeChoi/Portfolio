...
bool CObjectsShader::m_bShootState = true;
CPlayer *CObjectsShader::m_pPlayer = NULL;
//CCamera *CObjectsShader::m_pCamera = NULL;
list<CGameObject*>CObjectsShader::m_listBullet;
//CCamera *CObjectsShader::m_pCamera = NULL;
//list<CGameObject*>CObjectsShader::m_listBullet; // 총알
//CPlayer CGameObject::m_pPlayer = NULL;
CGameFramework::CGameFramework() {
	...
	for (int i = 0; i < m_nSwapChainBuffers; i++) m_nFenceValues[i] = 0; m_pScene = NULL;
...
void CGameFramework::CreateSwapChain() {
	...
	HRESULT hResult = m_pd3dDevice->CheckFeatureSupport(D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS, &d3dMsaaQualityLevels, sizeof(D3D12_FEATURE_DATA_MULTISAMPLE_QUALITY_LEVELS));
...
#ifdef _WITH_ONLY_RESIZE_BACKBUFFERS //전체화면 모드에서 바탕화면의 해상도를 바꾸지 않고 후면버퍼의 크기를 바탕화면 크기로 변경한다. 
...
{
	D3D12_RESOURCE_DESC d3dResourceDesk = {};
	d3dResourceDesk.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
	d3dResourceDesk.Width = m_nWndClientWidth;
	d3dResourceDesk.Height = m_nWndClientHeight;
	d3dResourceDesk.DepthOrArraySize = d3dResourceDesk.MipLevels = 1;
	d3dResourceDesk.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
	d3dResourceDesk.SampleDesc.Count = (m_bMsaa4xEnable) ? 4 : 1;
	d3dResourceDesk.SampleDesc.Quality = (m_bMsaa4xEnable) ? (m_nMsaa4xQualityLevels - 1) : 0;
	d3dResourceDesk.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
	d3dResourceDesk.Flags = D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
	...
	m_pd3dDevice->CreateCommittedResource(&d3dHeapProperties, D3D12_HEAP_FLAG_NONE, &d3dResourceDesk, D3D12_RESOURCE_STATE_DEPTH_WRITE, &d3dClearValue, __uuidof(ID3D12Resource),(void**)&m_pd3dDepthStencilBuffer);
...
void CGameFramework::CreateDirect3DDevice() {
	...
	hResult = ::CreateDXGIFactory1(__uuidof(IDXGIFactory4), (void **)&m_pdxgiFactory); //DXGI 팩토리를 생성한다.
	...
	hResult = m_pd3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, __uuidof(ID3D12Fence), (void **)&m_pd3dFence); m_nFenceValue = 1; //펜스를 생성하고 펜스 값을 1로 설정한다.
...
void CGameFramework::CreateCommandQueueAndList() {
	...
	d3dCommandQueueDesc.Flags = D3D12_COMMAND_QUEUE_FLAG_NONE; d3dCommandQueueDesc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT;
...
void CGameFramework::CreateRtvAndDsvDescriptorHeaps() {
	...
	d3dDescriptorHeapType = D3D12_DESCRIPTOR_HEAP_TYPE_DSV;
...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam) {
	...
	{ case WM_KEYUP:
		switch (wParam)
		{ 
			/*‘F1’ 키를 누르면 1인칭 카메라, ‘F2’ 키를 누르면 스페이스-쉽 카메라로 변경한다, ‘F3’ 키를 누르면 3인칭 카메라 로 변경한다.*/  
		case VK_F1: 
		case VK_F2: 
		case VK_F3:     
...
void CGameFramework::BuildObjects() 
{
	...
	m_pPlayer->SetOOBB(XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(3.0f, 3.0f, 3.0f), XMFLOAT4(0, 0, 0, 1));

	m_pCamera = m_pPlayer->GetCamera();
	//m_pCamera->SetOOBB(XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(1.0f, 1.0f, 1.0f), XMFLOAT4(0, 0, 0, 1));
	CObjectsShader::m_pPlayer = m_pPlayer;
...
{ 
	...
	//CCubeMeshDiffused *pWallMesh = new CCubeMeshDiffused(pd3dDevice, pd3dCommandList, 80.0f, 40.0f, 40.0f);
...
//
//			CCubeMeshDiffused *pBulletCubeMesh = new CCubeMeshDiffused(m_pd3dDevice, m_pd3dCommandList, 4.0f, 4.0f, 4.0f);
//			
...
//			bullet->SetMovingDirection(m_pCamera->GetLookVector());
...
//			CObjectsShader::m_listBullet = m_listBullet;
//			
//			/*cout << "------------------------" << endl;
...
//			//}
			...
			CObjectsShader::m_bShootState = true;
	...
	{
		m_bShootState = false;
		CObjectsShader::m_bShootState = false;
	}
		...
		if (cxDelta || cyDelta) 
	{
		...
		//m_pPlayer->SetOOBB(m_pPlayer->GetPosition(),)
...
void CGameFramework::AnimateObjects() 
{
	...
	if (m_pScene) m_pScene->AnimateObjects(m_GameTimer.GetTimeElapsed(),m_pd3dDevice, m_pd3dCommandList);
	//cout << "QBQBQBQB" << endl;
	...
	XMStoreFloat4(&m_pPlayer->m_xmOOBBTransformed.Orientation, XMQuaternionNormalize(XMLoadFloat4(&m_pPlayer->m_xmOfOBBTransformed.Orientation)));*/
		//cout << m_pCamera->GetLookVector().x << " \t " << m_pPlayer->GetLookVector().y << " \t " << m_pPlayer->GetLookVector().z << endl;
	...
	//cout << "-------------------------------" << endl;
	//cout << m_pPlayer->GetRightVector().x << "\t " << m_pPlayer->GetRightVector().y << "\t" << m_pPlayer->GetRightVector().z<< "\t" << endl;
	//cout << m_pPlayer->GetUpVector().x << "\t " << m_pPlayer->GetUpVector().y << "\t" << m_pPlayer->GetUpVector().z << "\t" << endl;
	//cout << m_pPlayer->GetLookVector().x << "\t " << m_pPlayer->GetLookVector().y << "\t" << m_pPlayer->GetLookVector().z << "\t" << endl;
	//cout << m_pPlayer->GetPosition().x << "\t " << m_pPlayer->GetPosition().y << "\t" << m_pPlayer->GetPosition().z << "\t" << endl;
	//cout << "-------------------------------" << endl;


	m_pPlayer->m_xmOOBBTransformed.Transform(m_pPlayer->m_xmOOBB, XMLoadFloat4x4(&XMFLOAT4X4(m_pPlayer->GetRightVector().x, m_pPlayer->GetRightVector().y, m_pPlayer->GetRightVector().z, 0, m_pPlayer->GetRightVector().x, m_pPlayer->GetUpVector().y, m_pPlayer->GetUpVector().z, 0, m_pPlayer->GetLookVector().x, m_pPlayer->GetLookVector().y, m_pPlayer->GetLookVector().z, 0, m_pPlayer->GetPosition().x, m_pPlayer->GetPosition().y, m_pPlayer->GetPosition().z, 1)));
	...
	/*m_pCamera->m_xmOOBBTransformed.Transform(m_pCamera->m_xmOOBB, XMLoadFloat4x4(&XMFLOAT4X4(m_pPlayer->GetCamera()->GetRightVector().x, m_pPlayer->GetCamera()->GetRightVector().y, m_pPlayer->GetCamera()->GetRightVector().z, 0, m_pPlayer->GetCamera()->GetUpVector().x, m_pPlayer->GetCamera()->GetUpVector().y, m_pPlayer->GetCamera()->GetUpVector().z, 0, m_pPlayer->GetCamera()->GetLookVector().x, m_pPlayer->GetCamera()->GetLookVector().y, m_pPlayer->GetCamera()->GetLookVector().z, 0, m_pPlayer->GetCamera()->GetPosition().x, m_pPlayer->GetCamera()->GetPosition().y, m_pPlayer->GetCamera()->GetPosition().z, 1)));
	XMStoreFloat4(&m_pCamera->m_xmOOBBTransformed.Orientation, XMQuaternionNormalize(XMLoadFloat4(&m_pCamera->m_xmOOBBTransformed.Orientation)));*/



	//cout << m_pCamera->m_xmOOBB.Center.x << "\t" << m_pCamera->m_xmOOBB.Center.y << "\t" << m_pCamera->m_xmOOBB.Center.z << endl;
	/*cout << "-------------------------------" << endl;
	cout << m_pCamera->GetViewMatrix()._11 << "\t " << m_pCamera->GetViewMatrix()._12 << "\t" << m_pCamera->GetViewMatrix()._13 << "\t" << m_pCamera->GetViewMatrix()._14 << endl;
	cout << m_pCamera->GetViewMatrix()._21 << "\t " << m_pCamera->GetViewMatrix()._22 << "\t" << m_pCamera->GetViewMatrix()._23 << "\t" << m_pCamera->GetViewMatrix()._24 << endl;
	cout << m_pCamera->GetViewMatrix()._31 << "\t " << m_pCamera->GetViewMatrix()._32 << "\t" << m_pCamera->GetViewMatrix()._33 << "\t" << m_pCamera->GetViewMatrix()._34 << endl;
	cout << m_pCamera->GetViewMatrix()._41 << "\t " << m_pCamera->GetViewMatrix()._42 << "\t" << m_pCamera->GetViewMatrix()._43 << "\t" << m_pCamera->GetViewMatrix()._44 << endl;
	cout << "-------------------------------" << endl;*/
	for (auto iter = /*CObjectsShader::*/m_listBullet.begin(); iter != /*CObjectsShader::*/m_listBullet.end(); ++iter)//총알 회전
	{
...
void CGameFramework::FrameAdvance() {
	...
	for (auto iter = /*cobjectsshader::*/m_listBullet.begin(); iter != /*cobjectsshader::*/m_listBullet.end(); ++iter)
	{
		(*iter)->Render(m_pd3dCommandList, m_pCamera);
