...
/////////////////////////////////////////////////////////////////////////////////////////////////////
//
CWallMesh::CWallMesh(float fWidth, float fHeight, float fDepth) : CMesh(16)
{
	float fHalfWidth = fWidth * 0.5f;
	float fHalfHeight = fHeight * 0.5f;
	float fHalfDepth = fDepth * 0.5f;
...
void CGameObject::Move(XMFLOAT3& vDirection, float fSpeed)
{
	...
	//m_xmOOBBTransformed.Transform(m_xmOOBB, XMLoadFloat4x4(&m_xmf4x4World)); //이걸 해줘야되는거 아닌가?
}

void CGameObject::Animate()
{
	if (m_fRotationSpeed != 0.0f) Rotate(m_xmf3RotationAxis, m_fRotationSpeed); //Rotate함수를 통해 사원수로 변경해주는것 같음
