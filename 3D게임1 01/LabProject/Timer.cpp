...
void CGameTimer::Tick(float fLockFPS = 0.0f)  //타이머 시간 갱신
{
...
//초당 프레임 수를 1 증가시키고 현재 프레임 처리 시간을 누적하여 저장한다.
	...
	{ 
		m_nCurrentFrameRate = m_nFramesPerSecond; m_nFramesPerSecond = 0; 
		m_fFPSTimeElapsed = 0.0f; 
	} 
...
unsigned long CGameTimer::GetFrameRate(LPTSTR lpszString = NULL, int nCharacters = 0) //프레임 레이트 반환
{
...
float CGameTimer::GetTimeElapsed() //프레임 평균경과 시간 반환
{
