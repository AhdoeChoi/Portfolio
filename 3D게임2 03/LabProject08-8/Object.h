...
public:
	...
	float m_fParticleDeleteTimer = 0;
public:
	bool ItisShooting;
...
public:
	...
	{
		SetPosition(m_xmf4x4World._41 + vDirection.x * fSpeed, m_xmf4x4World._42 + vDirection.y * fSpeed, m_xmf4x4World._43 + vDirection.z * fSpeed,true);
		//cout << " 유후 " << endl;
	}
...
public:
	...
	XMFLOAT4X4						m_xmf4x4ToParentTransform; //메쉬를 부모좌표계로 바꾸는 행렬 , 애니메이션할때 이놈을 바꿔주면 변환의 정보가 달라질꺼임
	XMFLOAT4X4						m_xmf4x4ToRootTransform; // 루트좌표계로 바꾸는거고, 그 루트좌표계에 있는 변환행렬을 사용하면 월드좌표계로 바뀌는거임
	XMFLOAT4X4						m_xmf4x4World;
...
public:
	...
	XMFLOAT3 GetPositionParent();
	XMFLOAT3 GetLook();
	XMFLOAT3 GetUp();
	XMFLOAT3 GetRight();

	void SetPosition(float x, float y, float z);
	void SetPosition(float x, float y, float z, bool a);
	...
	void Rotate(XMFLOAT3 *pxmf3Axis, float fAngle,bool a);
...
public:
	...
	CGameObject					*m_pHellfileMissileFrame = NULL; //미사일 쏠때 이거 활성화
};
