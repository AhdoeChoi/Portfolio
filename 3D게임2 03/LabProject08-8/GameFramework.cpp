...
void CGameFramework::BuildObjects()
{
	...
	m_pPlayer->SetOOBB(XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(4.0f, 4.0f, 4.0f), XMFLOAT4(0, 0, 0, 1));
	//m_pPlayer->m_pSibling = new CGameObject; 어케해야하지
	...
	m_pPlayer->SetPosition(XMFLOAT3(1708, 630, 1323));
...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		if (pKeysBuffer[0x51] & 0xF0) dwDirection |= DIR_UP; //q
		if (pKeysBuffer[0x5A] & 0xF0) dwDirection |= DIR_DOWN; //z
		if (pKeysBuffer[0x46] & 0xF0)// f키
		{
			m_pPlayer->SetPosition(XMFLOAT3(1708, 630, 1323));

		}
		if (pKeysBuffer[VK_SPACE] & 0x80) //총알 발사 0x80으로 바꾸면 한번만
		{
			//cout << m_bShootState << "!#$@#$@$" << endl;
			if (m_bShootState == false)
			{
				m_bShootState = true;
				m_pScene->SetBulletState(m_bShootState);
