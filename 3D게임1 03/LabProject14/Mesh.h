...
public:
	XMFLOAT3 PlaneNormalVectorRIGHT;
	XMFLOAT3 PlaneNormalVectorLEFT;
	XMFLOAT3 PlaneNormalVectorUP;
	XMFLOAT3 PlaneNormalVectorDOWN;
	XMFLOAT3 PlaneNormalVectorFRONT;
	XMFLOAT3 PlaneNormalVectorBACK;

	void SetPlaneNormalFRONT(XMFLOAT3 nvR) {  PlaneNormalVectorRIGHT = nvR; }
	XMFLOAT3 GetPlaneNormalRight() { return PlaneNormalVectorRIGHT; }
...
{
public: CAirplaneMeshDiffused
		(
			ID3D12Device *pd3dDevice,
			ID3D12GraphicsCommandList *pd3dCommandList,
			float fWidth = 20.0f, float fHeight = 20.0f,
			float fDepth = 4.0f,
...
protected:
	...
	즉, 실제 격자의 x-축 방향의 간격은 1이 아니 라 스케일 벡터의 x-좌표가 된다. 이렇게 하면 작은 격자(적은 정점)를 사용하더라도 큰 크기의 격자(지형)를 생성할 수 있다.*/
	XMFLOAT3 m_xmf3Scale;
public: 
