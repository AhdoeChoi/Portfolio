...
void CGameFramework::BuildObjects()
{
	...
	m_pPlayer->SetOOBB(XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(2.0f, 2.0f, 2.0f), XMFLOAT4(0, 0, 0, 1));
...
{ 
	...
	if (pKeyBuffer[0x50] & 0xF0) //p키
	{
		m_pPlayer->SetPosition(XMFLOAT3(1054, 249, 1645));
	}
	} 
...
LRESULT CALLBACK CGameFramework::OnProcessingWindowMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam) {
	...
	case WM_ACTIVATE:
	{
		if (LOWORD(wParam) == WA_INACTIVE)
			...
			break;
	} /*WM_SIZE 메시지는 윈도우가 생성될 때 한번 호출되거나 윈도우의 크기가 변경될 때 호출된다. 주 윈도우의 크기 를 사용자가 변경할 수 없으므로 윈도우의 크기가 변경되는 경우는 윈도우 모드와 전체화면 모드의 전환이 발생할 때 이다.*/
	case WM_SIZE: {
